...
##### Set all file paths and names #####
path <- "data/Bioinf/sequences/DClaar_2-34439409_seqs_KI_Compartment" # Path to where sequences are stored
refseqs <- "ITS2db_trimmed_derep_dada.fasta" # Path to ITS2 reference sequences
samdata <- "data/mapping_file_dada.txt" # Path to sample data/mapping file
fileout <- "analyses/dada2/KI_Compartment_dada.RData" # Output path and name

####################
# This script based off of https://benjjneb.github.io/dada2/ITS_workflow.html
...
filterAndTrim(fnFs, fnFs.filtN, fnRs, fnRs.filtN, 
              maxN = 0, # dada does not allow Ns, so must be zero
              multithread = TRUE, 
...
primerHits <- function(primer, fn) {
  ...
  nhits <- vcountPattern(primer, ShortRead::sread(ShortRead::readFastq(fn)), 
                         fixed = FALSE)
  return(sum(nhits > 0))
}
...
ref_seqs <- readDNAStringSet(filepath = refseqs)
min(width(ref_seqs))
max(width(ref_seqs))
...
taxa <- assignTaxonomy(seqtab.nochim, refseqs, multithread = TRUE, 
...
# Import to phyloseq
samdf <- read.table(samdata,header = TRUE) # Read in sample data
